...
def parse_geocaches(raw: str):
    ...
    for line in raw.splitlines():
        ...
        geocaches.append({"label": latex_escape(label), "url": url})
    return geocaches
...
def try_compile_pdf(tex_path: Path):
    ...
    for cmd in commands:
        try:
            subprocess.run(cmd, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=60)
...
st.caption("Genera schede Outdoor TN2G in stile editoriale, con GPX caricato su Google Drive.")
...
with st.form("event_form"):
    ...
    highlight = st.text_area("Testo box highlight iniziale", "Una camminata outdoor chill sopra Trento, tra bosco, social talk, viewpoint e mood TN2G.")
    ...
    come_arrivare = st.text_area("Come arrivare / partenza effettiva", "Inizieremo direttamente dal punto di ritrovo imboccando il percorso ad anello.\nCon i mezzi: indicare qui eventuali bus, fermate o tratto a piedi.")
    descrizione = st.text_area("Il percorso", "Si tratta di una breve escursione quasi urbana, con vista sulla città di Trento e su un contesto tranquillo e rilassante.\n\nÈ una proposta perfetta per un’uscita TN2G: scenica, sociale, accessibile e senza stress.")

    st.subheader("Programma e cose utili")
    programma = st.text_area("Programma della giornata", "15:00 Ritrovo\n15:15 Partenza\n16:30 Pausa panoramica\n18:30 Rientro previsto")
    cosa_portare = st.text_area("Cosa portare", "Scarpe comode\nAcqua\nSnack\nGiacca antivento\nTessera trasporti, se necessaria")
    sicurezza = st.text_area("Note sicurezza", "Percorso non tecnico. Prestare attenzione nei tratti sterrati e in caso di fondo umido.")
    meteo = st.text_area("Nota meteo", "Prima di partire controlliamo sempre il radar meteo.\nImportante: per leggere correttamente il radar, ricordate che l’orario può essere in UTC.")

    st.subheader("Mappa e note")
    note_mappa = st.text_area("Come leggere l'immagine, una voce per riga", "La linea colorata mostra il percorso previsto.\nIl grafico rappresenta il profilo altimetrico del giro.\nEventuali stelline o marker indicano punti di interesse lungo il percorso.")
    ...
    geocaching_intro = st.text_area("Testo introduttivo geocaching, opzionale", "Durante la passeggiata potremo anche cercare alcuni geocache nascosti lungo la zona. Per chi non lo conoscesse, il geocaching è una sorta di caccia al tesoro GPS.")
    geocaches = st.text_area("Codici geocache, uno per riga, opzionale", "")
    explorer_mission = st.text_area("Mini-missione TN2G, opzionale", "Se riusciamo a trovarli durante il giro, potremo considerarli come una piccola missione explorer della community. Perfetto mix tra outdoor, esplorazione e nerd mode.")

    st.subheader("Mood finale")
    mood_attivita = st.text_area("Mood dell'attività", "Questa uscita è pensata come un outdoor social leggero, adatto a chi ha voglia di camminare in compagnia, vedere un angolo bello sopra Trento e fare due chiacchiere senza stress.")
    ...
    upload_drive = st.checkbox("Carica automaticamente il GPX sul mio Google Drive e inserisci link cliccabile", value=True)
    compile_pdf = st.checkbox("Prova a compilare automaticamente il PDF, se LaTeX è installato", value=False)
...
if submitted:
    ...
    with open(zip_path, "rb") as f:
        st.download_button("Scarica pacchetto LaTeX ZIP", data=f, file_name=zip_path.name, mime="application/zip")

    if pdf_path and pdf_path.exists():
        with open(pdf_path, "rb") as f:
            st.download_button("Scarica PDF compilato", data=f, file_name=pdf_path.name, mime="application/pdf")
    elif compile_pdf:
