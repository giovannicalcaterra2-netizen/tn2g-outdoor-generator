...
def upload_gpx_to_drive(local_file: Path, desired_name: str):
    """
    Uploads GPX to a user-owned Google Drive folder shared with the service account.
    Returns a direct-download URL.
    """
    if service_account is None or build is None or MediaFileUpload is None:
        raise RuntimeError("Pacchetti Google API non installati. Controlla requirements.txt.")

    if not drive_configured():
        raise RuntimeError("Secrets Google Drive mancanti: gcp_service_account e drive_folder_id.")
...
st.caption("Genera schede Outdoor standardizzate. In V3 carica anche il GPX su Google Drive e inserisce il link nel PDF.")
...
with st.form("event_form"):
    ...
    descrizione = st.text_area("Descrizione breve", "Uscita outdoor tranquilla con camminata panoramica, pausa foto e momento chill finale.")
    programma = st.text_area("Programma della giornata", "14:30 Ritrovo\n14:45 Partenza\n16:00 Pausa panoramica\n18:30 Rientro previsto")
    ...
    meteo = st.text_area("Note meteo", "Controllare il meteo il giorno stesso.\nIn caso di peggioramento l'evento può essere modificato.")
    ...
    upload_drive = st.checkbox("Carica automaticamente il GPX su Google Drive e inserisci link cliccabile", value=True)
...
if submitted:
    ...
    if gpx_path and upload_drive:
        try:
            ...
            gpx_download_link = upload_gpx_to_drive(local_gpx, drive_name)
            st.success("GPX caricato su Google Drive e link generato.")
        except Exception as e:
            drive_error = str(e)
            st.warning(f"Pacchetto creato, ma upload GPX su Drive non riuscito: {drive_error}")
    ...
    data = {
        ...
        "drive_error": latex_escape(drive_error),
    }
    ...
    (event_dir / "README.txt").write_text(
        ...
        "Il GPX resta anche nella cartella uploads. Se configurato, il PDF include anche il link Drive cliccabile.\n",
        encoding="utf-8"
    )
